...
stupidSolver g@(Game {..}) = fmap fst $ listToMaybe $ sortBy (comparing (Down . snd)) $ map (\(a, b, _) -> ((a, b), predictScore (a, b))) $ filter (\(_, _, ctx) -> isNothing $ taken ctx) $ labEdges gameBoard
